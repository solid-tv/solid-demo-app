!function() {
    ...
    System.register([ "./render-legacy-t4wUAioc.js", "./api-legacy-BrsqDpuo.js", "./imagePerf-legacy-DKo3sP7L.js" ], function(_export, _context) {
        var createSignal, createElement, effect, onMount, setProp, createComponent, For, insert, insertNode, getImageUrl, api_default, createImagePerfRun;
        return {
            setters: [ function(_renderLegacy002Js) {
                ...
                For = _renderLegacy002Js.j, insert = _renderLegacy002Js.l, insertNode = _renderLegacy002Js.u;
            }, function(_apiLegacy00kJs) {
                getImageUrl = _apiLegacy00kJs.n, api_default = _apiLegacy00kJs.t;
            }, function(_imagePerfLegacy00GJs) {
                createImagePerfRun = _imagePerfLegacy00GJs.t;
            } ],
            execute: function() {
                _export("default", function() {
                    var _createSignal2 = _slicedToArray(createSignal([]), 2), images = _createSignal2[0], setImages = _createSignal2[1], _createSignal4 = _slicedToArray(createSignal(""), 2), setLoadTime = (_createSignal4[0], 
                    _createSignal4[1]), _createSignal6 = _slicedToArray(createSignal("Initializing..."), 2), status = _createSignal6[0], setStatus = _createSignal6[1], _createSignal8 = _slicedToArray(createSignal(0), 2), loadedCount = _createSignal8[0], setLoadedCount = _createSignal8[1], startTime = 0, imageLoadTimes = {}, perf = createImagePerfRun("image-40", 40), fetchImages = function() {
                        var n, _ref = (n = _regenerator().m(function _callee() {
                            var page1, page2, newImages, _t;
                            return _regenerator().w(function(_context2) {
                                for (;;) switch (_context2.p = _context2.n) {
                                  case 0:
                                    return _context2.p = 0, setStatus("Fetching images..."), _context2.n = 1, api_default.get("/movie/popular?page=1");

                                  case 1:
                                    return page1 = _context2.v, _context2.n = 2, api_default.get("/movie/popular?page=2");

                                  case 2:
                                    page2 = _context2.v, newImages = [].concat(_toConsumableArray(page1.results), _toConsumableArray(page2.results)).slice(0, 40).map(function(item) {
                                        return {
                                            id: item.id,
                                            src: perf.bust(getImageUrl(item.poster_path, "w185")),
                                            title: item.title
                                        };
                                    }), startTime = performance.now(), perf.start(), setImages(newImages), setStatus("Loading... 0/".concat(40)), 
                                    _context2.n = 4;
                                    break;

                                  case 3:
                                    _context2.p = 3, _t = _context2.v, console.error("Error fetching images:", _t), 
                    ...
                    var _el$, _el$2, _el$3, _el$4, _el$5, styles_container = {
                        width: 1920,
                        height: 1080,
                        x: 0,
                        y: 0
                    ...
                    insertNode(_el$, _el$3), setProp(_el$2, "autofocus", !0), insert(_el$2, createComponent(For, {
                        ...
                        children: function(item, index) {
                            var _el$6, col = index() % 8, row = Math.floor(index() / 8), x = 175 + 220.71 * col, y = 50 + 188.75 * row;
                            return _el$6 = createElement("view"), setProp(_el$6, "x", x), setProp(_el$6, "y", y), 
                            setProp(_el$6, "onEvent", {
                                loaded: function() {
                                    return function(index, id) {
                                        perf.onLoaded();
                                        var now = performance.now(), elapsed = now - startTime;
                                        imageLoadTimes[id] = elapsed;
                                        var currentCount = loadedCount() + 1;
                                        if (setLoadedCount(currentCount), setStatus("Loading... ".concat(currentCount, "/").concat(40)), 
                                        currentCount % 10 == 0 && performance.mark("images-loaded-".concat(currentCount)), 
                                        40 === currentCount) {
                                            var totalTime = now - startTime;
                                            setLoadTime("".concat(totalTime.toFixed(2), "ms")), setStatus("Loaded in ".concat(totalTime.toFixed(2), "ms")), 
                    ...
                    setProp(_el$4, "padding", [ 0, 20 ]), insert(_el$5, status), effect(function(_p$) {

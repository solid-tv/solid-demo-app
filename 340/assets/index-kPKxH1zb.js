...
(function(TextureType2) {
    ...
    TextureType2[TextureType2["group"] = 6] = "group";
...
class CoreNode extends EventEmitter {
    constructor(stage, props2) {
        ...
        const p = this.props = {
            ...
            clipping: props2.clipping,
            color: props2.color,
            colorTop: props2.colorTop,
            colorBottom: props2.colorBottom,
            colorLeft: props2.colorLeft,
            colorRight: props2.colorRight,
            colorTl: props2.colorTl,
            colorTr: props2.colorTr,
            colorBl: props2.colorBl,
            colorBr: props2.colorBr,
            scaleX: props2.scaleX,
            scaleY: props2.scaleY,
            rotation: props2.rotation,
            pivotX: props2.pivotX,
            pivotY: props2.pivotY,
            mountX: props2.mountX,
            mountY: props2.mountY,
            mount: props2.mount,
            pivot: props2.pivot,
            zIndex: props2.zIndex,
            textureOptions: props2.textureOptions,
            data: props2.data,
    ...
    updateLocalTransform() {
        ...
        if (texture && texture.dimensions && ((_a2 = p.textureOptions.resizeMode) == null ? void 0 : _a2.type) === "contain") {
    ...
    updateIsSimple() {
        var _a2;
        const p = this.props;
        this.isSimple = p.rotation === 0 && p.scaleX === 1 && p.scaleY === 1 && p.mountX === 0 && p.mountY === 0 && !(p.texture && ((_a2 = p.textureOptions.resizeMode) == null ? void 0 : _a2.type) === "contain");
    }
    ...
    set src(imageUrl) {
        ...
        if (this.props.group && imageUrl) {
            this.stage.txManager.registerGroupMember(this.props.group, imageUrl, this);
            return;
        }
    ...
    set group(value) {
        ...
        if (value && this.props.src) {
            this.stage.txManager.registerGroupMember(value, this.props.src, this);
        }
    }
    set imageType(type2) {
...
const _ImageTexture = class _ImageTexture extends Texture {
    ...
    static hasAlphaChannel(mimeType) {
        return mimeType.indexOf("image/png") !== -1;
    }
    static async loadImageFallback(txManager, src, hasAlpha) {
        const img = new Image;
        txManager.platform;
    ...
    static async createImageBitmap(txManager, blob, premultiplyAlpha, sx, sy, sw, sh) {
        const hasAlphaChannel = premultiplyAlpha != null ? premultiplyAlpha : _ImageTexture.hasAlphaChannel(blob.type);
        const imageBitmapSupported = txManager.imageBitmapSupported;
        const platform = txManager.platform;
        if (imageBitmapSupported.full === true && sw !== null && sh !== null) {
            const bitmap2 = await platform.createImageBitmap(blob, sx || 0, sy || 0, sw, sh, {
            ...
            return {
                data: await platform.createImageBitmap(blob),
                premultiplyAlpha: hasAlphaChannel
            };
        }
    ...
    static async load(txManager, src, premultiplyAlpha = true, sx = null, sy = null, sw = null, sh = null) {
        if (txManager.hasCreateImageBitmap === true) {
            if (isBase64Image(src) === false && txManager.hasWorker === true && txManager.imageWorkerManager !== null) {
                return txManager.imageWorkerManager.getImage(src, premultiplyAlpha, sx, sy, sw, sh);
            }
            let blob;
            ...
            return _ImageTexture.createImageBitmap(txManager, blob, premultiplyAlpha, sx, sy, sw, sh);
        }
        return _ImageTexture.loadImageFallback(txManager, src, premultiplyAlpha != null ? premultiplyAlpha : true);
    }
    async loadImage(src) {
        const {premultiplyAlpha: premultiplyAlpha, sx: sx, sy: sy, sw: sw, sh: sh} = this.props;
        return _ImageTexture.load(this.txManager, src, premultiplyAlpha, sx, sy, sw, sh);
    }
    async getTextureSource() {
        var _a2;
        let resp;
        ...
        if (typeof src !== "string") {
            if (src instanceof Blob) {
                if (this.txManager.hasCreateImageBitmap === true) {
                    ...
                    return _ImageTexture.createImageBitmap(this.txManager, src, premultiplyAlpha, sx, sy, sw, sh);
                } else {
                    return _ImageTexture.loadImageFallback(this.txManager, src, premultiplyAlpha != null ? premultiplyAlpha : true);
                }
            }
    ...
    static makeCacheKey(props2) {
        ...
        let cacheKey = "ImageTexture,".concat(key, ",").concat((_a2 = props2.premultiplyAlpha) != null ? _a2 : "true", ",").concat(props2.maxRetryCount);
...
class GroupTexture extends Texture {
    constructor(txManager, props2) {
        super(txManager);
        __publicField(this, "type", TextureType.group);
        __publicField(this, "props");
        this.props = props2;
    }
    async getTextureSource() {
        const {sources: sources, itemW: itemW, itemH: itemH, maxAtlasW: maxAtlasW} = this.props;
        const numItems = sources.length;
        const itemsPerRow = Math.floor(maxAtlasW / itemW);
        const numRows = Math.ceil(numItems / itemsPerRow);
        const atlasW = Math.min(maxAtlasW, itemsPerRow * itemW);
        const atlasH = numRows * itemH;
        const canvas2 = document.createElement("canvas");
        canvas2.width = atlasW;
        canvas2.height = atlasH;
        const ctx = canvas2.getContext("2d");
        if (!ctx) {
            throw new Error("Failed to get 2D context for GroupTexture");
        }
        const loadPromises = sources.map(async (src, index) => {
            const {data: data} = await ImageTexture.load(this.txManager, src);
            const x = index % itemsPerRow * itemW;
            const y = Math.floor(index / itemsPerRow) * itemH;
            if (data) {
                ctx.drawImage(data, x, y, itemW, itemH);
            }
        });
    ...
    static makeCacheKey(props2) {
        return "GroupTexture,".concat(props2.sources.join(","), ",").concat(props2.itemW, ",").concat(props2.itemH);
    }
    static resolveDefaults(props2) {
        var _a2, _b, _c, _d, _e;
        return {
            sources: (_a2 = props2.sources) != null ? _a2 : [],
            itemW: (_b = props2.itemW) != null ? _b : 0,
            itemH: (_c = props2.itemH) != null ? _c : 0,
            maxAtlasW: (_d = props2.maxAtlasW) != null ? _d : 2048,
            maxAtlasH: (_e = props2.maxAtlasH) != null ? _e : 1024
        };
    }
}

__publicField(GroupTexture, "z$__type__Props");
...
class CoreTextureManager extends EventEmitter {
    constructor(stage, settings) {
        ...
        __publicField(this, "maxRetryCount");
        __publicField(this, "uploadTextureQueue", new Set);
        __publicField(this, "initialized", false);
        __publicField(this, "stage");
        __publicField(this, "numImageWorkers");
        __publicField(this, "maxGroupAtlasW");
        __publicField(this, "maxGroupAtlasH");
        __publicField(this, "pendingGroups", new Map);
        __publicField(this, "platform");
        ...
        this.maxGroupAtlasW = (_a2 = settings.maxGroupAtlasW) != null ? _a2 : 2048;
        this.maxGroupAtlasH = (_b = settings.maxGroupAtlasH) != null ? _b : 1024;
        ...
        this.registerTextureType("GroupTexture", GroupTexture);
    ...
    async processSome(maxProcessingTime) {
        ...
        while (this.uploadTextureQueue.size > 0 && platform.getTimeStamp() - startTime < maxProcessingTime) {
    ...
    registerGroupMember(groupName, src, node) {
        let group = this.pendingGroups.get(groupName);
        if (!group) {
            group = [];
            this.pendingGroups.set(groupName, group);
            queueMicrotask(() => this.finalizeGroup(groupName));
        }
        group.push({
            src: src,
            node: node
    ...
    finalizeGroup(groupName) {
        const group = this.pendingGroups.get(groupName);
        if (!group || group.length === 0) return;
        this.pendingGroups.delete(groupName);
        const uniqueSources = [ ...new Set(group.map(m => m.src)) ];
        const firstNode = group[0].node;
        const itemW = firstNode.props.w;
        const itemH = firstNode.props.h;
        const groupTexture = this.createTexture("GroupTexture", {
            sources: uniqueSources,
            itemW: itemW,
            itemH: itemH,
            maxAtlasW: this.maxGroupAtlasW,
            maxAtlasH: this.maxGroupAtlasH
        });
        const itemsPerRow = Math.floor(this.maxGroupAtlasW / itemW);
        for (const member of group) {
            const {src: src, node: node} = member;
            const index = uniqueSources.indexOf(src);
            const x = index % itemsPerRow * itemW;
            const y = Math.floor(index / itemsPerRow) * itemH;
            node.texture = this.createTexture("SubTexture", {
...
class TextureMemoryManager {
    constructor(stage, settings) {
        ...
        __publicField(this, "orphanedTextures", new Set);
...
class CoreTextNode extends CoreNode {
    ...
    renderQuads(renderer2) {
        ...
        this.textRenderer.renderQuads(renderer2, this._cachedLayout, null, {
            ...
            width: this.props.w,
            height: this.props.h,
            parentHasRenderTexture: USE_RTT,
...
class Stage {
    ...
    resolveNodeDefaults(props2) {
        ...
        return {
            ...
            autosize: (_v = props2.autosize) != null ? _v : false,
            boundsMargin: (_w = props2.boundsMargin) != null ? _w : null,
            clipping: (_x = props2.clipping) != null ? _x : false,
            color: color,
            colorTop: colorTop,
            ...
            colorBl: colorBl,
            colorBr: colorBr,
            zIndex: (_y = props2.zIndex) != null ? _y : 0,
            parent: (_z = props2.parent) != null ? _z : null,
            texture: (_A = props2.texture) != null ? _A : null,
            textureOptions: (_B = props2.textureOptions) != null ? _B : {},
            shader: (_C = props2.shader) != null ? _C : this.defShaderNode,
            group: (_D = props2.group) != null ? _D : null,
...
class RendererMain extends EventEmitter {
    ...
    resolveTxSettings(textureMemory) {
        ...
        return {
            criticalThreshold: (_b = (_a2 = textureMemory == null ? void 0 : textureMemory.criticalThreshold) != null ? _a2 : currentTxSettings == null ? void 0 : currentTxSettings.criticalThreshold) != null ? _b : 2e8,
    ...
    setOptions(options) {
        ...
        if (options.deviceLogicalPixelRatio || options.devicePhysicalPixelRatio !== void 0) {
            ...
            (_a2 = this.inspector) == null ? void 0 : _a2.updateViewport(stageOptions.appWidth, stageOptions.appHeight, stageOptions.deviceLogicalPixelRatio);
...
function compareShaderProps(a, b) {
    if (a === b) return true;
...
class WebGlShaderProgram {
    ...
    reuseRenderOp(node, currentRenderOp) {
        ...
        if (shaderPropsA !== void 0 && shaderPropsB !== void 0) {
            for (const key in shaderPropsA) {
                if (!compareShaderProps(shaderPropsA[key], shaderPropsB[key])) {
                    return false;
                }
            }
        }
    ...
    bindRenderOp(renderOp) {
        ...
        renderOp.parentHasRenderTexture;
        isCoreNode && renderOp.parentHasRenderTexture ? renderOp.parentFramebufferDimensions : renderOp.framebufferDimensions;
        {
...
function computeLegacyObjectFit(node, img, resizeMode, clipX, clipY, srcPos, supportsObjectFit2, supportsObjectPosition2) {
    ...
    let fitType = (resizeMode == null ? void 0 : resizeMode.type) || (srcPos ? "none" : "fill");
    let drawW = naturalW;
    let drawH = naturalH;
    switch (fitType) {
      case "cover":
...
function updateNodeStyles(node) {
    ...
    let style = "position: absolute; z-index: ".concat(props2.zIndex, "; opacity: ").concat((_a2 = props2.alpha) != null ? _a2 : 1, ";");
    if (props2.clipping) {
    ...
    }
    {
        ...
        if (props2.mountX != null) {
            x -= ((_b = props2.w) != null ? _b : 0) * props2.mountX;
        }
        if (props2.mountY != null) {
            y -= ((_c = props2.h) != null ? _c : 0) * props2.mountY;
        }
        ...
        let pivotX = (_e = (_d = props2.pivotX) != null ? _d : props2.pivot) != null ? _e : .5;
        let pivotY = (_g = (_f = props2.pivotY) != null ? _f : props2.pivot) != null ? _g : .5;
        if (pivotX !== .5 || pivotY !== .5) {
            style += "transform-origin: ".concat(pivotX * 100, "% ").concat(pivotY * 100, "%;");
        }
    }
    if (node instanceof DOMText) {
        ...
        if (rawImgSrc) {
            ...
            const hasTint = props2.color !== 4294967295 && props2.color !== 0;
            if (hasTint) {
            ...
            const imgStyleParts = [ "position: absolute", "top: 0", "left: 0", "right: 0", "bottom: 0", "display: block", "pointer-events: none" ];
            if ((_h = props2.textureOptions.resizeMode) == null ? void 0 : _h.type) {
                ...
                const clipX = (_i = resizeMode.clipX) != null ? _i : .5;
                const clipY = (_j = resizeMode.clipY) != null ? _j : .5;
        ...
        if (((_k = props2.shader) == null ? void 0 : _k.props) != null) {
            let shaderProps = props2.shader.props;
            let borderWidth = shaderProps["border-w"];
            let borderColor = shaderProps["border-color"];
            let borderGap = (_l = shaderProps["border-gap"]) != null ? _l : 0;
            let borderAlign = (_m = shaderProps["border-align"]) != null ? _m : "inside";
            ...
            if (borderWidthHasValue && typeof borderColor === "number" && borderColor !== 0) {
                ...
                if (borderWidthIsNumber) {
                    ...
                    const topWidth = (_n = shaderProps["border-top"]) != null ? _n : borderWidth[0];
                    const rightWidth = (_o = shaderProps["border-right"]) != null ? _o : borderWidth[1];
                    const bottomWidth = (_p = shaderProps["border-bottom"]) != null ? _p : borderWidth[2];
                    const leftWidth = (_q = shaderProps["border-left"]) != null ? _q : borderWidth[3];
                ...
                if (colors.length > 0) {
                    ...
                    if (gradientStops) {
                        if (colors.length === 1) {
                            ...
                            const pivotX = ((_r = pivot[0]) != null ? _r : .5) * 100;
                            const pivotY = ((_s = pivot[1]) != null ? _s : .5) * 100;
                            let sizePart = "";
                            if (width > 0 && height > 0) {
            ...
            if (rawImgSrc) {
                if (!node.imgEl) {
                    ...
                    node.imgEl.addEventListener("load", () => {
                        ...
                        const resizeMode = (_a3 = node.props.textureOptions) == null ? void 0 : _a3.resizeMode;
                        const clipX = (_b2 = resizeMode == null ? void 0 : resizeMode.clipX) != null ? _b2 : .5;
                        const clipY = (_c2 = resizeMode == null ? void 0 : resizeMode.clipY) != null ? _c2 : .5;
                ...
                if (!srcPos && node.imgEl.complete && (!supportsObjectFit || !supportsObjectPosition) && node.imgEl.dataset.rawSrc === rawImgSrc) {
                    const resizeMode = (_t = node.props.textureOptions) == null ? void 0 : _t.resizeMode;
                    const clipX = (_u = resizeMode == null ? void 0 : resizeMode.clipX) != null ? _u : .5;
                    const clipY = (_v = resizeMode == null ? void 0 : resizeMode.clipY) != null ? _v : .5;
    ...
    if (textNodesToMeasure.size === 0) {
        if (typeof document !== "undefined" && "fonts" in document) {
            ...
            if (fonts2.status === "loaded") {
                setTimeout(updateDOMTextMeasurements);
            } else if (fonts2.ready && typeof fonts2.ready.then === "function") {
                fonts2.ready.then(updateDOMTextMeasurements);
            } else {
...
function startLightningRenderer(options, rootId = "app") {
    const enableDomRenderer = Config.domRendererEnabled;
...
function loadFonts(fonts2) {
    ...
    for (const font2 of fonts2) {
        ...
        } else if ("fontUrl" in font2) {
            if (enableDomRenderer) {
                loadFontToDom(font2);
            } else if (renderer$2.stage.renderer.mode !== "webgl") {
...
function calculateFlexOld(node) {
    var _a2, _b;
    ...
    let processableChildrenIndices = [];
    let hasOrder = false;
    let totalFlexGrow = 0;
    ...
    for (let idx = 0; idx < numProcessedChildren; idx++) {
        ...
        const marginStart = c[marginOne] || 0;
        const marginEnd = c[marginTwo] || 0;
        ...
        childMarginCrossStarts[idx] = c[crossMarginOne] || 0;
        childMarginCrossEnds[idx] = c[crossMarginTwo] || 0;
    ...
    if (totalFlexGrow > 0 && numProcessedChildren > 1) {
        ...
        if (availableSpace > 0) {
            for (let idx = 0; idx < numProcessedChildren; idx++) {
                ...
                if (flexGrowValue !== void 0 && flexGrowValue > 0) {
    ...
    if (justify === "flexStart") {
        if (node.flexWrap === "wrap" || isWrapReverse) {
            ...
            let crossCurrentPos = isWrapReverse ? containerCrossSize - childCrossSizeVar : 0;
            const crossGap = isRow ? (_a2 = node.columnGap) != null ? _a2 : gap : (_b = node.rowGap) != null ? _b : gap;
            for (let idx = 0; idx < numProcessedChildren; idx++) {
                const c = children2[processableChildrenIndices[idx]];
                if (currentPos + childTotalMainSizes[idx] > containerSize && currentPos > nodePadding) {
            ...
            const finalCrossSize = isWrapReverse ? containerCrossSize - crossCurrentPos : crossCurrentPos + childCrossSizeVar;
        ...
        currentPos = (containerSize - (totalItemSize + gap * (numProcessedChildren - 1))) / 2 + nodePadding;
        for (let idx = 0; idx < numProcessedChildren; idx++) {
            const c = children2[processableChildrenIndices[idx]];
            c[prop] = currentPos + childMarginStarts[idx];
            currentPos += childTotalMainSizes[idx] + gap;
        ...
        const spaceBetween = numProcessedChildren > 1 ? (containerSize - totalItemSize - nodePadding * 2) / (numProcessedChildren - 1) : 0;
        currentPos = nodePadding;
        for (let idx = 0; idx < numProcessedChildren; idx++) {
            const c = children2[processableChildrenIndices[idx]];
            c[prop] = currentPos + childMarginStarts[idx];
        ...
        const spaceAround = numProcessedChildren > 0 ? (containerSize - totalItemSize - nodePadding * 2) / numProcessedChildren : 0;
        ...
        const spaceEvenly = (containerSize - totalItemSize - nodePadding * 2) / (numProcessedChildren + 1);
...
const propagateKeyPress = (e, mappedEvent, isHold = false, isUp = false) => {
    ...
    const finalFocusElm = focusPath$1[0];
    ...
    if (mappedEvent) {
        eventHandlerKey = isUp ? "on".concat(mappedEvent, "Release") : "on".concat(mappedEvent);
    }
    if (!isUp) {
...
const calculateFlex = (__vite_import_meta_env__ == null ? void 0 : __vite_import_meta_env__.VITE_USE_NEW_FLEX) ? calculateFlexNew : calculateFlexOld;
...
function addToLayoutQueue(node) {
    ...
    if (!layoutRunQueued) {
        ...
        if ("reprocessUpdates" in renderer$2.stage && renderer$2.stage.reprocessUpdates) {
            renderer$2.stage.reprocessUpdates(runLayout);
        } else {
...
class ElementNode extends Object {
    ...
    set effects(v) {
        ...
        if (v.borderTop) parseAndAssignShaderProps("borderTop", v.borderTop, target);
        if (v.borderRight) parseAndAssignShaderProps("borderRight", v.borderRight, target);
        if (v.borderBottom) parseAndAssignShaderProps("borderBottom", v.borderBottom, target);
        if (v.borderLeft) parseAndAssignShaderProps("borderLeft", v.borderLeft, target);
        if (v.shadow) parseAndAssignShaderProps("shadow", v.shadow, target);
        if (this.rendered) {
            if (!this.lng.shader) {
                this.lng.shader = Config.convertToShader(this, target);
            } else if (Config.domRendererEnabled) {
    ...
    _sendToLightningAnimatable(name, value) {
        if (this.transition && this.rendered && Config.animationsEnabled && (this.transition === true || this.transition[name] || this.transition[getPropertyAlias(name)])) {
            ...
            return this.lng.animateProp(name, value, animationSettings || this.animationSettings || {});
        }
        this.lng[name] = value;
    }
    ...
    render(topNode) {
            ...
            if (Config.fontSettings) {
                for (const key in Config.fontSettings) {
                    if (textProps[key] === void 0) {
                        ...
                        if (key === "fontFamily" && textProps["fontWeight"] === void 0) {
                            value = "".concat(value).concat(Config.fontSettings.fontWeight || "");
                        }
                        textProps[key] = value;
                    }
                }
        ...
        const div = (_h = node.lng) == null ? void 0 : _h.div;
...
class SdfRenderOp extends CoreRenderOp {
    constructor(renderer2, shader, quadBufferCollection, worldAlpha, clippingRect, width, height, rtt, parentHasRenderTexture, framebufferDimensions) {
        super();
        __publicField(this, "renderer");
...
function isHTMLCanvasElement(obj) {
    return obj !== null && (typeof obj === "object" && obj.constructor && obj.constructor.name === "HTMLCanvasElement" || typeof HTMLCanvasElement !== "undefined" && obj instanceof HTMLCanvasElement);
}
...
class WebGlCtxTexture extends CoreContextTexture {
    ...
    async onLoadRequest() {
        ...
        if (isImageBitmap || tdata instanceof ImageData || isHTMLImageElement(tdata) === true || isHTMLCanvasElement(tdata) === true) {
            w = tdata.width;
            h = tdata.height;
...
class WebGlRenderer extends CoreRenderer {
    constructor(options) {
        ...
        __publicField(this, "surgicalView", new Float32Array(20));
    ...
    addQuad(node) {
        ...
        if (tx.type === TextureType.subTexture || tx.type === TextureType.group) {
            tx = tx.parentTexture || tx;
        }
    ...
    reuseRenderOp(node) {
        ...
        if (node.group !== curRenderOp.group) {
            return false;
        }
    ...
    addSdfQuads(glyphs, fontScale, transform, color, worldAlpha, distanceRange, atlasTexture, clippingRect, width, height, parentHasRenderTexture, framebufferDimensions, sdfShader2) {
        ...
        for (let gi = 0; gi < glyphCount; gi++) {
            ...
            const px1 = gx1 * fontScale;
            const py1 = gy1 * fontScale;
            const px2 = gx2 * fontScale;
            const py2 = gy2 * fontScale;
            const wx_tl = m0 * px1 + m3 * py1 + m6;
            const wy_tl = m1 * px1 + m4 * py1 + m7;
            const wx_tr = m0 * px2 + m3 * py1 + m6;
            const wy_tr = m1 * px2 + m4 * py1 + m7;
            const wx_bl = m0 * px1 + m3 * py2 + m6;
            const wy_bl = m1 * px1 + m4 * py2 + m7;
            const wx_br = m0 * px2 + m3 * py2 + m6;
            const wy_br = m1 * px2 + m4 * py2 + m7;
            f[idx] = wx_tl;
            f[idx + 1] = wy_tl;
            f[idx + 2] = u1;
            f[idx + 3] = v1;
            ...
            if (this.needsFullUpload || this.curBufferIdx > this.lastUploadedBufferSize) {
                ...
                for (let i = 0, len = renderList.length; i < len; i++) {
                    ...
                    if (node.isQuadDirty && node.quadBufferIndex !== -1) {
                        ...
                        const floatIdx = node.quadBufferIndex;
                        for (let j = 0; j < 20; j++) {
                            this.surgicalView[j] = this.fQuadBuffer[floatIdx + j];
                        }
                        glw.arrayBufferSubData(buffer, byteOffset, this.surgicalView);
...
const renderQuads$1 = (renderer2, layout, _vertexBuffer, renderProps) => {
    ...
    if (cache2 !== void 0 && cache2.vertices !== null) {
        ...
        if (cache2.layoutRef === layout && cache2.color === renderProps.color && cache2.alpha === renderProps.worldAlpha && ct[0] === t[0] && ct[1] === t[1] && ct[2] === t[3] && ct[3] === t[4]) {
            if (ct[4] !== t[6] || ct[5] !== t[7]) {
                const dx = t[6] - ct[4];
                const dy = t[7] - ct[5];
                const v = cache2.vertices;
            ...
            webGlRenderer.addSdfCachedQuads(cache2.vertices, cache2.glyphCount, ctxTexture, renderProps.clippingRect, renderProps.worldAlpha, layout.width, layout.height, renderProps.parentHasRenderTexture, renderProps.framebufferDimensions, sdfShader);
            return null;
        }
    }
    const startIdx = webGlRenderer.sdfBufferIdx;
    webGlRenderer.addSdfQuads(layout.glyphs, layout.fontScale, renderProps.globalTransform, renderProps.color, renderProps.worldAlpha, layout.distanceRange, ctxTexture, renderProps.clippingRect, layout.width, layout.height, renderProps.parentHasRenderTexture, renderProps.framebufferDimensions, sdfShader);
    if (cache2 !== void 0) {
        ...
        if (len > 0) {
            ...
            cache2.glyphCount = layout.glyphs.length;
...
const _storeKeepAlive = (map, element) => {
    if (map.has(element.id)) {
        console.warn('[KeepAlive] Element with id "'.concat(element.id, '" already in cache.'));
        return element;
    }
...
const createKeepAliveComponent = (map, storeFn) => props2 => {
    ...
    if (existing && (((_a2 = props2.shouldDispose) == null ? void 0 : _a2.call(props2, props2.id)) || ((_b = existing.children) == null ? void 0 : _b.destroyed))) {
        existing.children.destroy();
        existing.dispose();
...
const KeepAliveRoute = props2 => {
    ...
    const preload2 = props2.preload ? preloadProps => {
        ...
        if (existing && (((_a2 = props2.shouldDispose) == null ? void 0 : _a2.call(props2, key)) || ((_b = existing.children) == null ? void 0 : _b.destroyed))) {
            existing.children.destroy();
            existing.dispose();
        ...
        if (!existing) {
            ...
            (_c = existing.children) == null ? void 0 : _c.setFocus();
        }
    } : void 0;
    return createComponent(Route, mergeProps(props2, {
        preload: preload2,
        component: childProps => createComponent(KeepAliveRouteInternal, {
            id: key,
            onRemove: onRemove,
            onRender: onRender,
            get transition() {
...
const basePath$1 = "/solid-demo-app/340/";
...
const basePath = "/solid-demo-app/340/";
...
const Player = lazy(() => __vitePreload(() => import("./Player-GFGFI6df.js"), true ? [] : void 0));

const Grid = lazy(() => __vitePreload(() => import("./Grid-zZ9x5u5f.js"), true ? [] : void 0));

const Loops = lazy(() => __vitePreload(() => import("./Loops-O0eIoqQ6.js"), true ? [] : void 0));

const Infinite = lazy(() => __vitePreload(() => import("./Infinite-CXKWDDQX.js"), true ? [] : void 0));

const TMDBGrid = lazy(() => __vitePreload(() => import("./TMDBGrid-BUJjnIJA.js"), true ? [] : void 0));

const Portal = lazy(() => __vitePreload(() => import("./Portal-6fbZlBdE.js"), true ? [] : void 0));

const MatrixPage = lazy(() => __vitePreload(() => import("./Matrix-9-OeVgDy.js"), true ? [] : void 0));

const TextPage = lazy(() => __vitePreload(() => import("./Text-D8yOMtfH.js"), true ? [] : void 0));

const TextPosterPage = lazy(() => __vitePreload(() => import("./TextPoster-BLQkJkZ8.js"), true ? [] : void 0));

const CreatePage = lazy(() => __vitePreload(() => import("./Create-CISlVQyg.js"), true ? [] : void 0));

const ViewportPage = lazy(() => __vitePreload(() => import("./Viewport-C8w8NPyz.js"), true ? [] : void 0));

const PositioningPage = lazy(() => __vitePreload(() => import("./Positioning-UI1UKiD_.js"), true ? [] : void 0));

const LayoutPage = lazy(() => __vitePreload(() => import("./Layout-Cevk4zsK.js"), true ? [] : void 0));

const FocusBasicsPage = lazy(() => __vitePreload(() => import("./FocusBasics-Lpidwbfe.js"), true ? [] : void 0));

const KeyHandlingPage = lazy(() => __vitePreload(() => import("./KeyHandling-CWv8YdFY.js"), true ? [] : void 0));

const TransitionsPage = lazy(() => __vitePreload(() => import("./Transitions-CDWejApK.js"), true ? [] : void 0));

const ComponentsPage = lazy(() => __vitePreload(() => import("./Components-Bx2oCLSM.js"), true ? [] : void 0));

const FocusHandlingPage = lazy(() => __vitePreload(() => import("./FocusHandling-BWZ7NfwB.js"), true ? [] : void 0));

const GradientsPage = lazy(() => __vitePreload(() => import("./Gradients-BeTFW5yS.js"), true ? [] : void 0));

const FlexPage = lazy(() => __vitePreload(() => import("./Flex-baNhrejW.js"), true ? [] : void 0));

const FlexGrowPage = lazy(() => __vitePreload(() => import("./FlexGrow-B804qXiZ.js"), true ? [] : void 0));

const FlexMenuPage = lazy(() => __vitePreload(() => import("./FlexMenu-hRM-t6uY.js"), true ? [] : void 0));

const FlexSizePage = lazy(() => __vitePreload(() => import("./FlexSize-DRmlCuPH.js"), true ? [] : void 0));

const FlexColumnSizePage = lazy(() => __vitePreload(() => import("./FlexColumnSize-jI1Z0Whh.js"), true ? [] : void 0));

const FlexColumnPage = lazy(() => __vitePreload(() => import("./FlexColumn-BJL5R0pw.js"), true ? [] : void 0));

const ButtonsMaterialPage = lazy(() => __vitePreload(() => import("./ButtonsMaterial-BwMTi8ao.js"), true ? [] : void 0));

const SuperFlexPage = lazy(() => __vitePreload(() => import("./SuperFlex-CyJ84dx3.js"), true ? [] : void 0));

const Entity = lazy(() => __vitePreload(() => import("./Entity-CYUgSowy.js"), true ? [] : void 0));

const People = lazy(() => __vitePreload(() => import("./People-DlrshocK.js"), true ? [] : void 0));

const FireboltPage = lazy(() => __vitePreload(() => import("./Firebolt-C4NMCueY.js"), true ? [] : void 0));

const LoginPage = lazy(() => __vitePreload(() => import("./Login-BUrZtpmD.js"), true ? [] : void 0));

const VirtualPage = lazy(() => __vitePreload(() => import("./Virtual-DI_HTr34.js"), true ? [] : void 0));

const TagsPage = lazy(() => __vitePreload(() => import("./Tags-BphTmWNX.js"), true ? [] : void 0));

const ImagePerformance = lazy(() => __vitePreload(() => import("./ImagePerformance-8DG9B1Zz.js"), true ? [] : void 0));

const LargeImagePerformance = lazy(() => __vitePreload(() => import("./LargeImagePerformance-BKk9c6EE.js"), true ? [] : void 0));

const MixedImagePerformance = lazy(() => __vitePreload(() => import("./MixedImagePerformance-DFMfdAIG.js"), true ? [] : void 0));

const TextureCompressionPerformance = lazy(() => __vitePreload(() => import("./TextureCompressionPerformance-BIS0QBCd.js"), true ? [] : void 0));

const ComplexFlexPage = lazy(() => __vitePreload(() => import("./ComplexFlex-BIx7bu_U.js"), true ? [] : void 0));

const BenchmarkPage = lazy(() => __vitePreload(() => import("./Benchmark-CUFBMibM.js"), true ? [] : void 0));
...
Config.simpleAnimationsEnabled = !Config.domRendererEnabled;
                ...
                return [ createComponent(Route, {
                    ...
                    get children() {
                        return [ createComponent(Route, {
                            ...
                            path: "nested",
                            get component() {
                                return lazy(() => __vitePreload(() => import("./Nested-CV1NrQDg.js"), true ? [] : void 0));
                            }
                        }), createComponent(Route, {
                            path: "textposter",
                            ...
                            path: "keepalive",
                            get component() {
                                return lazy(() => __vitePreload(() => import("./KeepAlive-CzNDXnVJ.js"), true ? [] : void 0));
                            }
                        }), createComponent(Route, {
                            path: "suspense",
                            get component() {
                                return lazy(() => __vitePreload(() => import("./suspense-BWmDE_kS.js"), true ? [] : void 0));
                            }
                        }), createComponent(Route, {
                            path: "superflex",
...
export { convertItemsToTiles as $, ContentBlock as A, useFocusStack as B, Column as C, children as D, ElementNode as E, For as F, Announcer as G, memo as H, Index as I, styles$1 as J, assertTruthy as K, LazyRow as L, combineStyles as M, onCleanup as N, Block as O, Poster as P, createRoot as Q, Row as R, Show as S, Text as T, Dynamic as U, View as V, Button as W, MaterialButtonText as X, TileRow as Y, api as Z, getImageUrl as _, __vite_legacy_guard, createSignal as a, useParams as a0, Switch as a1, Match as a2, LazyColumn as a3, TitleRow as a4, setProp as a5, rootNode as a6, effect as a7, renderer$2 as a8, Hero as a9, AssetPanel as aa, resetCounter as ab, insertNode as ac, getOwner as ad, Config as ae, Suspense as af, createTextNode as ag, createResource as b, createComponent as c, createComputed as d, batch as e, createSelector as f, createEffect as g, hexColor as h, on as i, List as j, untrack as k, createMemo as l, mergeProps as m, createElement as n, onMount as o, chainRefs as p, hasFocus as q, use as r, setGlobalBackground as s, theme as t, useNavigate as u, spread as v, chainFunctions as w, insert as x, debounce as y, activeElement as z };

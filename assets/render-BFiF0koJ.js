...
var CoreNode = class extends EventEmitter {
    constructor(stage, props) {
        ...
        const {texture: texture, shader: shader, src: src, rtt: rtt, boundsMargin: boundsMargin, parent: parent} = props;
        ...
        if (rtt !== false) this.rtt = rtt;
        if (boundsMargin !== null) this.boundsMargin = boundsMargin;
    ...
    addChild(node, previousParent = null) {
        const inRttCluster = true && (this.props.rtt === true || this.parentHasRenderTexture === true);
        const children = this.children;
        ...
        if (previousParent !== null) {
            const previousParentInRttCluster = true && (previousParent.props.rtt === true || previousParent.parentHasRenderTexture === true);
    ...
    applyValues(anim) {
        ...
        if (anim.shaderProps && anim.node.shader) {
            ...
            for (const key in anim.shaderProps) {
                ...
                if (anim.progress === 1) anim.node.shader.props[key] = value.target; else if (anim.progress === 0) anim.node.shader.props[key] = value.start; else if (value.isColor) anim.node.shader.props[key] = mergeColorProgress(value.start, value.target, easedProgress); else anim.node.shader.props[key] = value.start + (value.target - value.start) * easedProgress;
                updated = true;
            }
...
var CoreTextureManager = class extends EventEmitter {
    ...
    initialize(support) {
        ...
        this.initialized = true;
        this.emit("initialized");
        if (this.uploadTextureQueue.size > 0) this.processUntil(Infinity).catch(err => {
    ...
    async uploadQueued(texture) {
        ...
        try {
            if (texture.textureData === null) await texture.getTextureData();
            if (this.isTextureDead(texture)) return;
...
var CoreTextNode = class extends CoreNode {
    constructor(stage, props, textRenderer) {
        ...
        this._type = "sdf";
        this.onTextureLoaded = (_, dimensions) => {
            if (true && this.parentHasRenderTexture) this.notifyParentRTTOfUpdate();
            if (dimensions.w > 1 && dimensions.h > 1) this.emit("loaded", {
                type: "texture",
                dimensions: dimensions
            });
...
var Stage = class {
    ...
    drawFrame(hasActiveAnimations = false) {
        ...
        if (this.txManager.hasUpdates() === true) (hasActiveAnimations === true ? this.txManager.processOne() : this.txManager.processUntil(this.options.textureProcessingTimeLimit)).catch(err => {
        ...
        renderer.render();
        if (sampling === true) {
            const renderEnd = platform.getTimeStamp();
            ...
            const frameRenderMs = renderEnd - renderStart;
...
export { onMount as $, For as A, createContext as B, isElementNode as C, createWebGLContext as Ct, Config as D, isInteger as E, Switch as F, createRoot as G, createMemo as H, batch as I, getListener as J, createSelector as K, children as L, Match as M, Show as N, activeElement as O, Suspense as P, onCleanup as Q, createComponent$1 as R, hasFocus as S, assertTruthy as St, isFunction as T, createRenderEffect as U, createEffect as V, createResource as W, mergeProps as X, getOwner as Y, on as Z, ElementNode as _, calcFactoredRadiusArray as _t, createElement as a, useContext as at, loadFonts as b, TextureType as bt, effect as c, defaultFontMetrics as ct, memo as d, CoreShaderNode as dt, resetErrorBoundaries as et, rootNode as f, RenderTexture as ft, use as g, UpdateType as gt, spread as h, CoreNodeRenderState as ht, createComponent as i, untrack as it, Index as j, $TRACK as k, insert as l, mapTextLayout as lt, setProp as m, uploadCompressedTexture as mt, Text as n, sharedConfig as nt, createRenderer as o, takeCanvasPrefetch as ot, scheduleTask as p, SubTexture as pt, createSignal as q, View as r, startTransition as rt, createTextNode as s, takeSdfPrefetch as st, Dynamic as t, runWithOwner as tt, insertNode as u, normalizeFontMetrics as ut, focusPath as v, compareRect as vt, isFocused as w, mergeColorAlpha as wt, renderer$1 as x, DIRTY_QUAD_BUFFER as xt, useFocusManager as y, getNormalizedRgbaComponents as yt, createComputed as z };
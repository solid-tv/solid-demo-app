import { $ as onMount, A as For, a as createElement, c as effect, i as createComponent, l as insert, m as setProp, q as createSignal, s as createTextNode, u as insertNode } from "./render-CAUp88iy.js";
...
var Card = props => {
    ...
    insert(_el$2, () => String(props.index), null), insertNode(_el$4, _el$5), setProp(_el$4, "color", 65535), 
    setProp(_el$4, "padding", [ 0, 8 ]), setProp(_el$4, "borderRadius", 4), setProp(_el$4, "height", 40), 
    setProp(_el$4, "display", "flex"), setProp(_el$4, "alignItems", "center"), insertNode(_el$5, createTextNode("Button")), 
    setProp(_el$5, "fontSize", 16), setProp(_el$5, "color", 4294967295), effect(_$p => setProp(_el$, "color", focused() ? 858993561 : 858993663, _$p)), 
    _el$;
...
function ComplexFlex() {
    ...
    const items = Array.from({
        length: 8
    }).map((_, i) => i), initialRows = Array.from({
        length: 8
    }).map((_, i) => i), [rows, setRows] = createSignal(initialRows);
    return onMount(() => {
        ...
        console.log(`ComplexFlex Render duration: ${null == measure ? void 0 : measure.duration}ms`);
        const sleep = ms => new Promise(resolve => setTimeout(resolve, ms));
        (async () => {
            await sleep(100);
            for (let i = 0; i < 3; i++) console.log(`Resetting rows to [] (Recreation step ${i + 1})`), 
            setRows([]), await sleep(100), console.log(`Recreating rows to full (Recreation step ${i + 1})`), 
            setRows(initialRows), await sleep(100);
        })();

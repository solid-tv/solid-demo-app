import { V as createEffect, a as createElement, i as createComponent, l as insert, m as setProp, q as createSignal, s as createTextNode, u as insertNode } from "./render-C0sQE9My.js";

import { t as Row } from "./Row-DD7YRHoz.js";

import { n as setGlobalBackground } from "./state-D1XDNvBT.js";
...
function send(message) {
    ...
    if (method.match(/^on[A-Z]/)) if (json.params.listen) eventMap[json.id] = module.toLowerCase() + "." + method[2].toLowerCase() + method.substr(3); else Object.keys(eventMap).forEach(key => {
...
function mock(module, method, params, value, contextParameterCount, def) {
    ...
    const key = `${module}.${method}${contextParameterCount ? "." + Object.keys(params).filter(key => key !== "value").map(key => params[key]).join(".") : ""}`;
...
var dotGrab = (obj = {}, key) => {
    const keys = key.split(".");
    for (let i = 0; i < keys.length; i++) obj = obj[keys[i]] = obj[keys[i]] !== void 0 ? obj[keys[i]] : {};
...
var LegacyTransport = class LegacyTransport {
    constructor(bridge) {
        this.bridge = bridge;
    }
    send(msg) {
...
function transform(result, transforms) {
    ...
    Object.keys(methods).forEach(key => {
        ...
        const [module, method] = method_info["x-method"].split(".");
        const params = {};
...
var Transport = class Transport {
    ...
    _sendAndGetId(module, method, params, transforms) {
        const {promise: promise, json: json, id: id} = this._processRequest(module, method, params, transforms);
    ...
    _addPromiseToQueue(module, method, params, transforms) {
        return new Promise((resolve, reject) => {
            ...
            if (deprecated) console.warn(`WARNING: ${module}.${method}() is deprecated. ` + deprecated.alternative);
            if (method.match(/^on[A-Z]/)) if (params.listen) this._eventIds.push(this._id); else this._eventIds = this._eventIds.filter(id => id !== this._id);
        });
    }
    static get() {
    ...
    receiveHandler(message) {
        ...
        if (p) {
            if (json.error) p.reject(json.error); else {
                ...
                if (p.transforms) if (Array.isArray(json.result)) result = result.map(x => Results_default.transform(x, p.transforms)); else result = Results_default.transform(result, p.transforms);
                p.resolve(result);
            }
            delete this._promises[json.id];
        }
        if (this._eventIds.includes(json.id) && !isEventSuccess(json.result)) this._eventEmitters.forEach(emit => {
...
var doListen = function(module, event, callback, context, once, internal = false) {
    init();
    if (typeof callback !== "function") return Promise.reject("No valid callback function provided."); else {
        ...
        const events = wildcard ? validEvents[module] : [ event ];
        const promises = [];
        ...
        events.forEach(event => {
            ...
            if (Object.values(listeners.get(key)).length === 0) {
                ...
                const {id: id, promise: promise} = Transport.listen(module, "on" + event[0].toUpperCase() + event.substring(1), args);
                keys[id] = key;
                promises.push(promise);
            }
            const setter = internal ? listeners.setInternal : listeners.set;
            if (wildcard) setter(key, "" + listenerId, value => callback(event, value)); else setter(key, "" + listenerId, callback);
        });
        let resolve, reject;
        ...
        if (promises.length) promises.reduce((prevPromise, currentPromise) => prevPromise.then(() => currentPromise).then(responses => {
...
var doClear = function(moduleOrId = false, event = false, context) {
    ...
    if (typeof moduleOrId === "number") {
        ...
        return false;
    } else if (!moduleOrId && !event) listeners.keys().forEach(key => {
        listeners.removeKey(key);
        unsubscribe(key);
    }); else if (!event) listeners.keys().forEach(key => {
...
function prop(moduleName, key, params, callbackOrValue = void 0, immutable, readonly, contextParameterCount) {
    ...
    const type = Router_default(params, callbackOrValue, contextParameterCount);
